<section class="home-section">
  <div class="text">Edit Schedule</div>
<!-- <pre>print_r($teachers, true) </pre> -->
  ...
  <div class="teacher-container">
    ...
    @endif
        ...
        <form action="{{ route('mio.UpdateSchedule', ['id' => $editdata['scheduleid']]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="table-header">
                ...
                <button type="button" class="btn cancel-btn">
                    <a href="{{ route('mio.ViewSchedule') }}">Cancel</a>
                </button>
                <button type="submit" class="btn add-btn">
                    <span class="icon">✔</span> Update Schedule
                </button>
                </div>
            </div>
            ...
            <div class="form-container">
                ...
                <div class="form-row">
                    ...
                    <label>Schedule ID <span style="color: red">*</span></label>
                    <input type="text" name="scheduleid" id="scheduleID" value="{{ $editdata['scheduleid'] }}" required />
                    </div>
                    <div class="form-group">
                    <label>Schedule Name <span style="color: red">*</span></label>
                    <input type="text" name="schedule_name" value="{{ $editdata['schedule_name'] }}" required />
                    </div>
                </div>

                <div class="form-row">
                    <div class="form-group">
                    <label>Schedule Code <span style="color: red">*</span></label>
                    <input type="text" name="schedule_code" value="{{ $editdata['schedule_code'] }}" required />
                    </div>

                    <div class="form-group">
                    <label>Schedule Type <span style="color: red">*</span></label>
                    <select name="schedule_type" required>
                        <option value="academic" {{ $editdata['schedule_type'] === 'academic' ? 'selected' : '' }}>Academics</option>
                        <option value="specialized" {{ $editdata['schedule_type'] === 'specialized' ? 'selected' : '' }}>Specialized</option>
                        <option value="admin_support" {{ $editdata['schedule_type'] === 'admin_support' ? 'selected' : '' }}>Administrative and Support</option>
                    </select>
                    </div>
                ...
                <div class="form-row">
                    ...
                    <textarea name="description" rows="3" style="resize: none; height: 100px;">{{ $editdata['description'] }}</textarea>
                    </div>
                </div>
                </div>

                <!-- Assign to Teacher -->
                <div class="section-header">Assign to Teacher</div>
                <div class="section-content">
                <div class="form-row">
                    <div class="form-group" style="flex: 1;">
                    <label>Head Teacher <span style="color: red">*</span></label>
                    <select name="teacherid" required id="select-teacher" class="schedule-selector">
                        <option value="">Select a Teacher</option>
                        @foreach($teachers as $teacher)
                        <option value="{{ $teacher['teacherid'] }}"
                            {{ $editdata['teacherid'] === $teacher['teacherid'] ? 'selected' : '' }}>
                ...
                <div class="form-row">
                    ...
                    <select name="student_ids[]" id="select-students" class="schedule-selector" multiple>
                        ...
                        <option value="{{ $student['id'] }}"
                            {{ in_array($student['id'], $editdata['student_ids'] ?? []) ? 'selected' : '' }}>
                ...
                <div class="form-row">
                    <div class="form-group">
                    <label>
                        <input type="checkbox" name="sameTimeToggle" id="sameTimeToggle"
                        {{ isset($editdata['common_start_time']) ? 'checked' : '' }} />
                ...
                <!-- Common Time -->
                <div id="common-time" class="form-row" style="display: none;">
                    <div class="form-group">
                    <label>Start Time <span style="color: red">*</span></label>
                    <input type="time" name="common_start_time"
                        value="{{ $editdata['common_start_time'] ?? '' }}" />
                    </div>
                    <div class="form-group">
                    <label>End Time <span style="color: red">*</span></label>
                    <input type="time" name="common_end_time"
                        value="{{ $editdata['common_end_time'] ?? '' }}" />
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group" style="flex: 1;">
                    <label>Occurrences (Days of Week) <span style="color: red">*</span></label>
                    <select name="occurrences[]" id="occurrences" multiple required style="height: 170px;">
                        @php
                        $selectedDays = $editdata['occurrences'] ?? [];
                        @endphp
                        @foreach(['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'] as $day)
                        <option value="{{ $day }}" {{ array_key_exists($day, $selectedDays) ? 'selected' : '' }}>{{ $day }}</option>
                ...
                <!-- Per Day Times -->
                <div id="individual-times">
                    @if(!isset($editdata['common_start_time']))
                    @foreach($selectedDays as $dayName => $times)
                        <div class="form-row">
                            <label style="font-weight: bold; margin-top: 10px;">{{ $dayName }}</label>
                            <div class="form-group">
                                <label>Start Time for {{ $dayName }}</label>
                                <input type="time" name="day_times[{{ $dayName }}][start]" value="{{ $editdata['day_times'][$dayName]['start'] ?? $times['start_time'] ?? '' }}" />
                            </div>
                            <div class="form-group">
                                <label>End Time for {{ $dayName }}</label>
                                <input type="time" name="day_times[{{ $dayName }}][end]" value="{{ $editdata['day_times'][$dayName]['end'] ?? $times['end_time'] ?? '' }}" />
                            </div>
                        </div>
                    @endforeach
...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    const sameTimeToggle = document.getElementById('sameTimeToggle');
    const commonTimeSection = document.getElementById('common-time');
    const isSameTime = sameTimeToggle.checked;
    commonTimeSection.style.display = isSameTime ? 'flex' : 'none';
  });
</script>
