<section class="home-section">
    <div class="text">
        ...
        <div class="breadcrumb-item active" style="font-size: 1.3rem;">Picture Activity</div>
    ...
    <main class="main-assignment-content">
        @if (!empty($speech))
                ...
                @foreach ($activities as $activityId => $activity)
                    <div class="assignment-card activity-toggle" onclick="toggleActivityDetails('{{ $activityId }}')">
                        ...
                        <div class="activity-actions" style="margin-top: 10px; display: flex; gap: 10px;">
                            ...
                            <!-- Delete Button -->
                            <form action="{{ route('mio.subject-teacher.speech-picture.delete', ['subjectId' => request()->route('subjectId'), 'difficulty' => $difficulty, 'activityId' => $activityId]) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this activity?');" style="margin: 0;">
                                @csrf
                                @method('DELETE')
            ...
            @endforeach
        @else
            <p>No phrase activities found.</p>
        @endif
    ...
    <div id="addActivityModal" class="modal assignment-modal" style="display: none;">
        <div class="modal-content">
            ...
            <h2>Add Picture Activity</h2>
            <form action="{{ route('mio.subject-teacher.speech-picture.add', ['subjectId' => request()->route('subjectId')]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <button type="button" onclick="addPhraseItem()">+ Add Picture</button>
    ...
    <div id="editActivityModal" class="modal assignment-modal" style="display: none;">
        <div class="modal-content">
            ...
            <h2>Edit Picture Activity</h2>
            <form id="editActivityForm" method="POST" action="{{ route('mio.subject-teacher.speech-picture.edit', ['subjectId' => request()->route('subjectId')]) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <button type="button" onclick="addEditPhraseItem()">+ Add Picture</button>
            ...
            if (item.image_path) {
                imagePreview = `
                    <div style="margin-top:5px;">
                        ...
                        <img src="${item.image_url}" alt="Preview" style="max-width: 100px; max-height: 80px; border:1px solid #ccc; margin-top: 5px;">
                    </div>
                `;
            }
