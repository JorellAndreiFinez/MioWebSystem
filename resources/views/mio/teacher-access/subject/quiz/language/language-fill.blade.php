            ...
            <form action="{{ route('mio.subject-teacher.language-fill.add', ['subjectId' => request()->route('subjectId')]) }}" method="POST" enctype="multipart/form-data">
                ...
                <div id="phrase-items-container">
                    ...
                    <div class="phrase-item" data-index="0">
                        ...
                        <div id="image-preview-0" style="margin-top: 5px;"></div>
                        ...
                        <!-- ✅ DISTRACTORS SECTION -->
                        <div id="distractors-container-0">
                            <label>Distractors</label>
                            <div class="distractor-input" style="display: flex; align-items: center; gap: 5px;">
                                <input type="text" name="items[0][distractors][]" placeholder="Enter distractor">
                                <button type="button" onclick="removeDistractor(this)">🗑️</button>
                            </div>

                        </div>
                        <button type="button" onclick="addDistractor(0)">+ Add Distractor</button>
...
<script>
        ...
        function addPhraseItem() {
            ...
            <hr>
                <div class="phrase-item" data-index="${phraseIndex}">
                    ...
                    <!-- ✅ DISTRACTORS SECTION -->
                    <div id="distractors-container-${phraseIndex}">
                        <label>Distractors</label>
                        <div class="distractor-input">
                            <input type="text" name="items[${phraseIndex}][distractors][]" placeholder="Enter distractor">
                        </div>
                    </div>
                    <button type="button" onclick="addDistractor(${phraseIndex})">+ Add Distractor</button>
        ...
        function addDistractor(index) {
            const distractorContainer = document.getElementById(`distractors-container-${index}`);
            if (!distractorContainer) return;

            const distractorInputHTML = `
                <div class="distractor-input" style="margin-top: 5px; display: flex; align-items: center; gap: 5px;">
                    <input type="text" name="items[${index}][distractors][]" placeholder="Enter distractor">
                    <button type="button" onclick="removeDistractor(this)">🗑️</button>
                </div>
            `;

            distractorContainer.insertAdjacentHTML('beforeend', distractorInputHTML);
        }
...
function addEditPhraseItem(item = {}) {
    ...
    distractors.forEach(distractor => {
        distractorsHTML += `
            <div class="distractor-input" style="display: flex; align-items: center; gap: 5px; margin-top: 5px;">
                <input type="text" name="items[${index}][distractors][]" value="${distractor}" placeholder="Enter distractor">
                <button type="button" onclick="removeDistractor(this)">🗑️</button>
            </div>
        `;
    });

    // Default empty distractor if none exist
    ...
    const itemHTML = `
        <div class="phrase-item" data-index="${index}">
            <input type="text" name="items[${index}][sentence]" placeholder="Enter phrase text" value="${textValue}" required>
            ...
            <!-- ✅ DISTRACTORS -->
            <div id="edit-distractors-container-${index}">
                <label>Distractors</label>
                ${distractorsHTML}
            </div>
            <button type="button" onclick="addEditDistractor(${index})">+ Add Distractor</button>
...
function handleEditButtonClick(element) {
    ...
    const formattedItems = Object.values(items).map(i => ({
            sentence: i.sentence ?? i.text ?? '',
            image_url: i.image_url ?? '',
            audio_url: i.audio_url ?? '',
            distractors: i.distractors ?? []
        }));

    populateEditItems(formattedItems);
