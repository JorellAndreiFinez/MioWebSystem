<section class="home-section">
                ...
                @foreach ($activities as $activityId => $activity)
                    <div class="assignment-card activity-toggle" onclick="toggleActivityDetails('{{ $activityId }}')">
                        ...
                        <div class="activity-actions" style="margin-top: 10px; display: flex; gap: 10px;">
                            ...
                            <!-- Delete Button -->
                            <form action="{{ route('mio.subject-teacher.auditory-bingo.delete', ['subjectId' => request()->route('subjectId'), 'difficulty' => $difficulty, 'activityId' => $activityId]) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this activity?');" style="margin: 0;">
                                @csrf
                                @method('DELETE')
                        ...
                        <div class="phrase-items-grid" id="activity-{{ $activityId }}" style="display: none;">
                            @if(isset($activity['items']) && is_array($activity['items']))
                                ...
                                @endforeach
                            @else
                                <p>No bingo items found for this activity.</p>
                            @endif
                        </div>
    ...
    <div id="addActivityModal" class="modal assignment-modal" style="display: none;">
        <div class="modal-content">
            ...
            <h2>Add Phrase Activity</h2>
            <form action="{{ route('mio.subject-teacher.auditory-bingo.add', ['subjectId' => request()->route('subjectId')]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                    ...
                    <div class="phrase-item" data-index="0">
                            ...
                            <div style="display: flex; align-items: center; gap: 10px; margin-top: 10px;">
                                ...
                                <label style="display: flex; align-items: center; gap: 5px; margin: 0;">
                                    <input type="checkbox" name="items[0][is_correct]" value="1">
                                    Correct
                                </label>
                            </div>
                        </div>
    ...
    <div id="editActivityModal" class="modal assignment-modal" style="display: none;">
        <div class="modal-content">
            ...
            <form id="editActivityForm" method="POST" action="{{ route('mio.subject-teacher.auditory-bingo.edit', ['subjectId' => request()->route('subjectId')]) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
            ...
            const itemHTML = `
                <div class="phrase-item" data-index="${phraseIndex}">
                    ...
                    <div style="display: flex; align-items: center; gap: 10px; margin-top: 10px;">
                        <button type="button" class="remove-item-btn" onclick="removePhraseItem(this)">🗑️</button>
                        <label style="display: flex; align-items: center; gap: 5px; margin: 0;">
                            <input type="checkbox" name="items[${phraseIndex}][is_correct]" value="1">
                            Correct
                        </label>
                    </div>
                </div>
    ...
    const itemHTML = `
        <div class="phrase-item" data-index="${index}">
            ...
            <div style="display: flex; align-items: center; gap: 10px; margin-top: 10px;">
                <button type="button" class="remove-item-btn" onclick="removePhraseItem(this)">🗑️</button>
                <label style="display: flex; align-items: center; gap: 5px; margin: 0;">
                    <input type="checkbox" name="items[${index}][is_correct]" value="1" ${checked}>
                    Correct
                </label>
            </div>
        </div>
...
function populateEditItems(items = {}, correctAnswers = []) {
    const container = document.getElementById('edit-phrase-items-container');
    container.innerHTML = '';
    editPhraseIndex = 0;

    const correctSet = new Set(correctAnswers.map(c => c.image_id));

    Object.entries(items).forEach(([itemId, data]) => {
        addEditPhraseItem({
            text: data.filename?.replace(/\.[^/.]+$/, '') ?? '',
            is_correct: correctSet.has(itemId),
            image_url: data.image_url ?? '',  // Add if stored in Firebase or server
            audio_url: data.audio_url ?? ''   // Add if stored in Firebase or server
        });
    });
}
